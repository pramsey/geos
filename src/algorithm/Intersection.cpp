...
Intersection::intersectionLineSegment(
    const geom::Coordinate& line1,
    const geom::Coordinate& line2,
    const geom::Coordinate& seg1,
    const geom::Coordinate& seg2)
{
    ...
    if ((orientS1 > 0 && orientS2 > 0) || (orientS1 < 0 && orientS2 < 0)) {
        return geom::Coordinate::getNull();
    }
     ...
     */
    geom::Coordinate intPt = intersection(line1, line2, seg1, seg2);
    if (!intPt.isNull())
        return intPt;
